...
import {
    RainbowAllianceToken,
    RainbowAllianceToken__factory,
...
describe("DAO Attack Exercise 1", function () {
    ...
    before(async function () {
        ...
        const RainbowAllianceTokenFactory = (await ethers.getContractFactory(
            ...
            deployer
        )) as RainbowAllianceToken__factory;
    ...
    it("Test governance", async function () {
        ...
        await expect(
            rainbowAlliance
                .connect(user3)
                .createProposal("Donate 1000$ to charities")
        ).to.be.revertedWith("no voting rights");

        // Should be able to create proposals if you have voting power
        await expect(
            await rainbowAlliance.createProposal(
                "Pay 100$ to george for a new Logo"
            )
        ).to.not.be.reverted;

        // Can't vote twice
        await expect(rainbowAlliance.vote(1, true)).to.be.revertedWith(
            "already voted"
        );

        // Shouldn't be able to vote without voting rights
        await expect(
            rainbowAlliance.connect(user3).vote(1, true)
        ).to.be.revertedWith("no voting rights");

        // Non existing proposal, reverts
        await expect(rainbowAlliance.vote(123, false)).to.be.revertedWith(
